...
class HotelsDAO(BaseDAO):
    ...
    async def find_all(
        ...
        booked_hotels AS (
            SELECT hotel_id,
            SUM(rooms.quantity - COALESCE(rooms_booked, 0)) AS rooms_left
            FROM rooms
        ...
        get_hotels_with_rooms = (
            ...
            # Таким образом алхимия отдает все столбцы по одному, как отдельный
            # атрибут. Если передать всю модель Hotels и один дополнительный
            # столбец rooms_left, то будет проблематично для Pydantic
            # распарсить такую структуру данных. То есть проблема кроется
            # именно в парсинге ответа алхимии Пайдентиком.
            select(
